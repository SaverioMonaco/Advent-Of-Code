...
. (You can't reverse a pattern by flipping a towel upside-down, as that would 
...
function load(file_path)
   ...
   # p_towel_available = split.(split(data[1], ", "), "")
   p_towel_available = split(data[1], ", ")

   p_towel_target = data[3:end]
   
   return p_towel_available, p_towel_target
end

file_path = "./data/day19.txt"

(p_towel_available, p_towel_target) = load(file_path)

function starts_the_same(towel_target, towel_available)
    if length(towel_available) <= length(towel_target) && towel_available == towel_target[1:length(towel_available)]
        return true 
    else
        return false
    end
...
function foo(target, p_available)
    p_candidate = filter(x -> starts_the_same.(target, x), p_available)
    p_towel_target_temp = [target[length(candidate)+1:end] for candidate in p_candidate]
    return p_towel_target_temp
end

function get_n_towel_possible(p_towel_available, p_towel_target)
    n_towel_possible = 0
    
    # for towel_target in p_towel_target
    for (i, towel_target) in zip(range(1, length(p_towel_target)), p_towel_target)
        # println("$i/$(length(p_towel_target))")
        p_towel_available_temp = []
        for available in p_towel_available
            if occursin(available, towel_target)
                push!(p_towel_available_temp, available)
            end
        end
        # println(p_towel_available_temp, " ",  length(p_towel_available_temp))
        ...
        while length(p_towel_target_temp) > 0
            ...
            # println(p_towel_target_temp)
            p_towel_target_temp = [foo(target_temp, p_towel_available_temp) for target_temp in p_towel_target_temp]
            p_towel_target_temp = unique([item for subarray in p_towel_target_temp for item in subarray])
            # println(length(p_towel_available_temp))
            # println(p_towel_target_temp)
            if "" in p_towel_target_temp
                n_towel_possible += 1
                break
            end
        end
    ...
    return n_towel_possible
end

@time solution = get_n_towel_possible(p_towel_available, p_towel_target)
...
function get_n_arrangments(p_towel_available, p_towel_target)
    n_arrangements = 0
    ...
    for (i, towel_target) in zip(range(1, length(p_towel_target)), p_towel_target)
        ...
        while length(p_towel_target_temp) > 0
            ...
            for (target_temp, weight) in zip(p_towel_target_temp, p_weight)
                p_t = foo(target_temp, p_towel_available_temp)
                for t in p_t
                    if t == ""
                        n_arrangements += weight
                    elseif t in p_towel_target_temp_new
                        p_weight_new[findfirst(x -> x == t, p_towel_target_temp_new)] += weight
                    else
                        push!(p_towel_target_temp_new, t)
                        push!(p_weight_new, weight)
                    end
                end
            end
            p_towel_target_temp = p_towel_target_temp_new
...
(p_towel_available, p_towel_target) = load(file_path)
@time solution = get_n_arrangments(p_towel_available, p_towel_target)
println("Solution is $solution")