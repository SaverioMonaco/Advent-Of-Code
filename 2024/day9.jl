...
more contiguous free space by compacting all of the files, but his program isn'
t working; you offer to help.
...
12345 has three files: a one-block file with ID 0, a three-block file with ID 1
, and a five-block file with ID 2. Using one character for each block where 
...
function unravel(data)
    data_int = parse.(Int, split(data, ""))
    
    data_unravel = []
    for i in range(1, length(data_int))
        value = data_int[i]
        if i % 2 == 0
            append!(data_unravel, repeat([""], value))
        else
            id = div(i, 2)
            append!(data_unravel, repeat([id], value))
        end
    end
...
function filesort(data)
    while true
        # println(data)
        free_idx = findfirst(x -> x == "", data)
        # println(free_idx)
        if isnothing(free_idx)
           return data
        end
        data[free_idx] = data[end]
        data = data[1:end-1]
    end
...
function checksum(data)
    mult_list = range(0, length(data)-1)
    return sum(data .* mult_list)
end


data = load(path)
data2 = unravel(data)
data2 = filesort(data2)

solution = checksum(data2)
...
The process of updating the filesystem checksum is the same; now, this example'
s checksum would be 2858.
...
function unravel(data)
    ...
    for i in range(1, length(data_int))
        ...
        if i % 2 == 0
            if value > 0
                push!(data_unravel, join(repeat(".", value)))
            end
        else
            id = div(i, 2)
            push!(data_unravel, repeat([id], value))
        end
    end
    ...
    while idx > 1
        ...
        if !('.' in data[idx])
            for idx2 in range(1, idx)
                length_value = length(data[idx])
                length_leftover = length(data[idx2]) - length_value
                if ('.' in data[idx2]) && (length_leftover >= 0)
                    data[idx2] = data[idx]
                    data[idx]  = repeat('.', length_value)
                    if length_leftover > 0
                        to_insert = repeat('.', length_leftover)
                        insert!(data, idx2+1, to_insert)
                        idx += 1
                    end
                    break 
                end
...
for d1 in data2
    for d2 in d1
        push!(flatten_data2, d2)
    end
end

function checksum(data)
    dot_index = findall(x -> x == '.', data)  # Locate indices of "."
    data[dot_index] .= 0  # Replace "." with 0 (use broadcasting with .=)
    # Generate indices 0 to length(data)-1
    return sum((0:length(data)-1) .* data)  # Sum the weighted values
end

solution = checksum(flatten_data2)