...
# Load the txt file 
data = readdlm("./data/day1.txt")

# Split into two arrays
p_loc1 = sort(data[: , 1])
p_loc2 = sort(data[: , 2])

# A function to compute the L1 distance
function distance(x::Number, y::Number)
 return abs(x - y)   
end

# Apply distance between each i-th element
# of the two arrays
p_dist = distance.(p_loc1, p_loc2)

# np.sum basically
sum_p_dist = reduce((x, y) -> x + y, p_dist)
...
function newsimilarity(num, list_other)
    return count(x -> x == num, list_other)
end

newsimilarity_score = 0

for loc1 in p_loc1
    newsimilarity_score += newsimilarity(loc1, p_loc2)
end

using Printf; @printf("%.0f", newsimilarity_score)