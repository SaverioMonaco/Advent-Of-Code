...
function load(path :: String)
    data = parse.(Int, hcat(split.(readlines(path), "") ...))
    return data
end

data = load(file_path)

function hike(map, path, x, y)
    candidates = []
    # step up 
    if y > 1 && (map[x,y] - map[x,y-1] == -1)
        if !([x, y-1] in path)
            push!(candidates, [x, y-1])
            push!(path, [x, y-1])
        end
    end
    # step right 
    if x < size(map,1) && (map[x,y] - map[x+1,y] == -1)
        if !([x+1, y] in path)
            push!(candidates, [x+1, y])
            push!(path, [x+1, y])
        end
    end
    # step left 
    if x > 1 && (map[x,y] - map[x-1,y] == -1)
        if !([x-1, y] in path)
            push!(candidates, [x-1, y])
            push!(path, [x-1, y])
        end
    ...
    # step down 
    if y < size(map,2) && (map[x,y] - map[x,y+1] == -1)
        if !([x, y+1] in path)
            push!(candidates, [x, y+1])
            push!(path, [x, y+1])
        end
...
function get_score(map)
    score = 0
    for x in range(1, size(map,1))
        for y in range(1, size(map,2))
            if map[x,y] == 0
                hike_path = [[x,y]]
                (candidates, hike_path) = hike(map, hike_path, x, y)
                while length(candidates) > 0
                    # println(path, candidates)
                    new_candidates = []
                    for candidate in candidates
                        new_candidate, hike_path = hike(map, hike_path, candidate[1], candidate[2])
                        append!(new_candidates, new_candidate)
                    end
                    candidates = new_candidates
                end
                println(length(unique(hike_path)))
                for (x,y) in unique(hike_path)
                    if map[x,y] == 9
                        score += 1
                    end
                end
...
function get_score(map)
    ...
    for x in range(1, size(map,1))
        for y in range(1, size(map,2))
            if map[x,y] == 0
                candidates = hike(map, x, y)
                while length(candidates) > 0
                    # println(path, candidates)
                    new_candidates = []
                    for candidate in candidates
                        new_candidate = hike(map, candidate[1], candidate[2])
                        append!(new_candidates, new_candidate)
                        for new_c in new_candidate
                            if map[new_c[1], new_c[2]] == 9
                                score += 1
                            end
                        end
                    end
