...
. Furthermore, numbers in the equations cannot be rearranged. Glancing into 
...
Only three of the above equations can be made true by inserting operators:
    ...
    value: 81 + 40 * 27 and 81 * 40 + 27 both equal 3267 (when evaluated left-to-right)!
...
function load(path :: String)
    function subsplit(row)
        return [parse(Int, row[1]), parse.(Int, split(row[2], " "))]
    end

    data = split.(readlines(path), ": ")    
    data = subsplit.(data)
    
    return data
end

function get_ops_bin(row)
    n_ops = 2^(length(row[2])-1)
    return digits.(range(0, n_ops-1), base=2, pad=length(row[2])-1)
end

function make_op(row, opbitstring)
    result = row[2][1]
    for (value, opbitchar) in zip(row[2][2:end], opbitstring)
        if opbitchar == 0
            result += value
        elseif opbitchar == 1
            result *= value
...
function is_valid(row)
    p_opbitstring = get_ops_bin(row)
    return any(x -> x, make_op.((row,), p_opbitstring))
end

data = load("./data/day7.txt")

p_good_op = is_valid.(data)

result = 0
for (good_op, row) in zip(p_good_op, data)
    if good_op == 1
        result += row[1]
    end
end
...
function elephant(int1, int2)
    conc_string = parse(Int, string(int1) * string(int2))
    return conc_string
end

function get_ops_tri(row)
    n_ops = 3^(length(row[2])-1)
    return digits.(range(0, n_ops-1), base=3, pad=length(row[2])-1)
end

function make_op_tri(row, opbitstring)
    result = row[2][1]
    for (value, opbitchar) in zip(row[2][2:end], opbitstring)
        if opbitchar == 0
            result += value
...
function is_valid_tri(row)
    p_opbitstring = get_ops_tri(row)
    return any(x -> x, make_op_tri.((row,), p_opbitstring))
end

data = load("./data/day7.txt")
p_good_op = is_valid_tri.(data)
