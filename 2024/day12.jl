...
type E plants are in a region of area 3; the type D plants are in a region of area 1.
...
function get_n_neighbours(map, region, x, y)
    n_neighbours = 0
    if y > 1 && ([x, y-1] in region.p_cord)
        n_neighbours += 1
    end
    # step right 
    if x < size(map,1) && ([x+1, y] in region.p_cord)
        n_neighbours += 1
    end
    # step left 
    if x > 1 && ([x-1, y] in region.p_cord)
        n_neighbours += 1
    end
    # step down 
    if y < size(map,2) && ([x, y+1] in region.p_cord)
        n_neighbours += 1
    end
    return n_neighbours
end

function diffuse(map, region, x, y)
    new_plants = []
    # step up 
    if y > 1 && (map[x,y] == map[x,y-1]) && !([x, y-1] in region.p_cord)
            push!(new_plants, [x, y-1])
            push!(region.p_cord, [x, y-1])
            region.area += 1
            n_neighbours = get_n_neighbours(map, region, x, y-1)
            if n_neighbours == 1
                # +--+    +--+--+
                # |  | -> |     |
                # +--+    +--+--+
    ...
    # step right 
    if x < size(map,1) && (map[x,y] == map[x+1,y]) && !([x+1, y] in region.p_cord)
            push!(new_plants, [x+1, y])
            push!(region.p_cord, [x+1, y])
            region.area += 1
            n_neighbours = get_n_neighbours(map, region, x+1, y)
            if n_neighbours == 1
                # +--+    +--+--+
                # |  | -> |     |
                # +--+    +--+--+
    ...
    # step left 
    if x > 1 && (map[x,y] == map[x-1,y]) && !([x-1, y] in region.p_cord)
            push!(new_plants, [x-1, y])
            push!(region.p_cord, [x-1, y])
            region.area += 1
            n_neighbours = get_n_neighbours(map, region, x-1, y)
    ...
    # step down 
    if y < size(map,2) && (map[x,y] == map[x,y+1]) && !([x, y+1] in region.p_cord)
            push!(new_plants, [x, y+1])
            push!(region.p_cord, [x, y+1])
            region.area += 1
            n_neighbours = get_n_neighbours(map, region, x, y+1)
...
function get_region(x, y, map)
    new_region = Region(map[x,y], [[x,y]], 4, 1)
    p_cord_buf = [[x,y]]
    while length(p_cord_buf) > 0
        new_p_cord_buf = []
        for cord_buf in p_cord_buf
            new_cord_buf, new_region = diffuse(map, new_region, cord_buf[1], cord_buf[2])
            append!(new_p_cord_buf, new_cord_buf)
        end
        p_cord_buf = unique(new_p_cord_buf)
    end
    return new_region
end
...
function get_p_corn(map, region, cord)
    ...
    if cord[1] > 1
        plant_l = map[cord[1]-1, cord[2]]
    else
        plant_l = "#"
    end
    if cord[1] < size(map,1)
        plant_r = map[cord[1]+1, cord[2]]
    else
        plant_r = "#"
    end
    if cord[2] > 1
        plant_u = map[cord[1], cord[2]-1]
    else
        plant_u = "#"
    end
    if cord[2] < size(map,2)
        plant_d = map[cord[1], cord[2]+1]
    else
        plant_d = "#"
    end
    
    if plant_u != plant_o && plant_l != plant_o
        push!(p_corn, "$(cord[1]) $(cord[2])")
    end
    if plant_l != plant_o && plant_d != plant_o
        push!(p_corn, "$(cord[1]) $(cord[2]+1)")
    end
    if plant_d != plant_o && plant_r != plant_o
        push!(p_corn, "$(cord[1]+1) $(cord[2]+1)")
    end
    if plant_r != plant_o && plant_u != plant_o
        push!(p_corn, "$(cord[1]+1) $(cord[2])")
    end
    ...
    for x in range(1, size(map,1))
        for y in range(1, size(map,2))
            if !([x, y] in traced) 
                ...
                solution += region.area * n_sides
                # println("$(region.plant): $(n_sides)")
                # println("  $(unique(p_corn))")
            end
        end
    end
    return solution
